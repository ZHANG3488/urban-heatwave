...
def _export_config_to_environment() -> dict:
    """Read config.yaml beside this script and expose values to legacy code.

    This adapter changes only path/runtime configuration. The scientific
    algorithms below are unchanged.
    """
    import yaml
    ...
    path_values = {
        "UHR_PAIR_CSV": paths.get("pair_csv"),
        "UHR_PAIR_FALLBACK": paths.get("pair_fallback", paths.get("pair_csv")),
        "UHR_STATION_META": paths.get("station_metadata"),
        "UHR_ISD_BASE_DIR": paths.get("isd_base_dir"),
        "UHR_ERA5_STATION_DIR": paths.get("era5_station_dir"),
        "UHR_KG_TIF": paths.get("koppen_geiger_tif"),
        "UHR_CONTINENT_SHP": paths.get("continent_shapefile"),
        "UHR_BV_CSV": paths.get("building_volume_csv"),
        "UHR_ANALYSIS_OUTPUT_DIR": paths.get(
            "analysis_output_dir",
            str(Path(unified_root) / "analysis" / "main_multiyear"),
        ),
    }

    option_values = {
        "UHR_YEARS": ",".join(str(v) for v in options.get("years", range(2015, 2025))),
        "UHR_BOOT_N": options.get("bootstrap_n", 1000),
        "UHR_RANDOM_SEED": options.get("random_seed", 42),
        "UHR_N_WORKERS": options.get("n_workers"),
        "UHR_USE_ERA5_CLIMATOLOGY": options.get("use_era5_climatology", True),
        "UHR_MIN_LOYO_REF_YEARS": options.get("min_loyo_ref_years", 5),
        "UHR_BV_RADIUS": options.get("building_volume_radius", "1000m"),
    }

    for name, value in {**path_values, **option_values}.items():
...
def _env_int_list(name: str, default):
    ...
    if not years:
        raise ValueError(f"{name} did not contain any valid integers")
    return years


def _env_optional_int(name: str, default=None):
    value = os.environ.get(name)
...
KG_TIF = os.environ.get(
    "UHR_KG_TIF",
    "/home/yuxizhang/code/gzz/koppen_geiger_data/1991_2020/koppen_geiger_0p00833333.tif",
)
...
# 1. Global Config
# ─────────────────────────────────────────────────────────────
YEARS = _env_int_list("UHR_YEARS", range(2015, 2025))
...
USE_ERA5_CLIMATOLOGY = _env_bool("UHR_USE_ERA5_CLIMATOLOGY", True)

ERA5_STATION_DIR = os.environ.get(
    "UHR_ERA5_STATION_DIR",
    "/home/yuxizhang/code/gzz/final_code/impact/final/other_result/era5_station_output_nc_only_fast/station_daily",
)
# ERA5 站点 Tmax CSV 命名约定：{USAF}-{WBAN}_{station_type}_tmax.csv
# 必须包含 "date"（YYYY-MM-DD）和 "tmax"（℃）两列

MIN_LOYO_REF_YEARS = int(os.environ.get("UHR_MIN_LOYO_REF_YEARS", "5"))
...
PAIR_CSV_PATH = os.environ.get(
    "UHR_PAIR_CSV",
    "/home/yuxizhang/code/gzz/annual_ml_dataset_2024_new.csv",
)
STATION_META_PATH = os.environ.get(
    "UHR_STATION_META",
    "/home/yuxizhang/code/gzz/all_station_analysis/Extract_Final_Integrated_2023.csv",
)
PAIR_FALLBACK_PATH = os.environ.get("UHR_PAIR_FALLBACK", PAIR_CSV_PATH)
ISD_BASE_DIR = os.environ.get("UHR_ISD_BASE_DIR", "/home/yuxizhang/isd_data")
OUTPUT_DIR = os.environ.get(
    "UHR_ANALYSIS_OUTPUT_DIR",
    "/home/yuxizhang/code/gzz/final_code/impact/final/code_era/Final_code/latest_full_analysis_plot_code/unified_outputs/analysis/main_multiyear",
)
CONTINENT_SHP = os.environ.get(
    "UHR_CONTINENT_SHP",
    "/home/yuxizhang/code/gzz/all_station_analysis/World_Continents_-8107292174417139505/World_Continents.shp",
)
BV_CSV_PATH = os.environ.get(
    "UHR_BV_CSV",
    "/home/yuxizhang/code/gzz/all_station_analysis/stations_building_volume_100m.csv",
)
BV_RADIUS = os.environ.get("UHR_BV_RADIUS", "1000m")
...
BOOT_N      = int(os.environ.get("UHR_BOOT_N", "1000"))
RANDOM_SEED = int(os.environ.get("UHR_RANDOM_SEED", "42"))
N_WORKERS = _env_optional_int("UHR_N_WORKERS", None)
...
def main():
    ...
    runtime_config = {
        ...
        "pair_fallback": PAIR_FALLBACK_PATH,
        "isd_base_dir": ISD_BASE_DIR,
        "era5_station_dir": ERA5_STATION_DIR,
        "kg_tif": KG_TIF,
        "continent_shp": CONTINENT_SHP,
        "building_volume_csv": BV_CSV_PATH,
        "output_dir": OUTPUT_DIR,
        "use_era5_climatology": USE_ERA5_CLIMATOLOGY,
        "bootstrap_n": BOOT_N,
        "random_seed": RANDOM_SEED,
        "n_workers": n_cores,
    }
    with open(os.path.join(OUTPUT_DIR, "runtime_config.json"), "w", encoding="utf-8") as f:
        import json as _json
        _json.dump(runtime_config, f, ensure_ascii=False, indent=2)
