...
# -*- coding: utf-8 -*-
"""
Compute station-pair heatwave and non-heatwave diurnal-cycle metrics.

This script reads station-pair metadata, ISD-Lite observations, optional
ERA5-derived daily Tmax references, continent polygons and auxiliary urban
surface variables, then writes the pair-level tables used by the manuscript
figures and supplementary analyses.

Input data are not distributed with this repository. Configure paths with the
command-line options shown by ``python 01_compute_pair_period_metrics.py --help``.
"""
...
KG_TIF = "data/koppen_geiger/koppen_geiger_0p00833333.tif"
...
# KG main groups used for stratified mechanism and mediation analyses.
...
def extract_koppen_codes(lons, lats, tif_path=None):
    if tif_path is None:
        tif_path = KG_TIF
    out = []
...
# ── Minimal period-specific raw diurnal-support QC ───────────
# These two non-overlapping windows partition all 24 local hours.
# The core-night window is kept separate so that observations at
# 20:00–23:59 cannot mask complete absence of 00:00–07:59 data.
TEMP_QC_NIGHT_HOURS = tuple(range(0, 8))
TEMP_QC_COMPLEMENT_HOURS = tuple(range(8, 24))
TEMP_QC_ALL_HOURS = tuple(range(24))

# Each station-period must contain at least one genuinely observed
# temperature value in each window on at least this fraction of its
# contributing local dates. Short-gap interpolation does not count.
TEMP_QC_MIN_RAW_WINDOW_DAY_FRAC = 0.50
TEMP_QC_MIN_RAW_HOUR_BINS_PER_WINDOW = 1

# Preserve the pre-existing requirement of at least eight temperature-
# supported climatological hour bins before final cross-bin filling.
TEMP_QC_MIN_SUPPORTED_HOUR_BINS = 8

TEMP_DIURNAL_QC_VERSION = "raw_two_window_support_v1"

assert (
    set(TEMP_QC_NIGHT_HOURS)
    | set(TEMP_QC_COMPLEMENT_HOURS)
) == set(TEMP_QC_ALL_HOURS)
assert not (
    set(TEMP_QC_NIGHT_HOURS)
    & set(TEMP_QC_COMPLEMENT_HOURS)
)

# Method-protocol labels written to output records for provenance.
...
ERA5_STATION_DIR = "data/era5_station_daily"
...
PAIR_CSV_PATH = "data/station_pairs.csv"
STATION_META_PATH = (
    "data/station_metadata.csv"
)
PAIR_FALLBACK_PATH = PAIR_CSV_PATH
ISD_BASE_DIR = "data/isd_lite"
OUTPUT_DIR = "outputs/analysis/main_multiyear"
CONTINENT_SHP = (
    "data/continents/World_Continents.shp"
)
BV_CSV_PATH = (
    "data/stations_building_volume_100m.csv"
)
BV_RADIUS = "1000m"
...
# Absolute heatwave thresholds by continent.
CONTINENT_THRESHOLD = {
...
# ─────────────────────────────────────────────────────────────
# 5. ISD-Lite Reader  [dew-point parsing]
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# 6. Data Loading  [temperature and dew-point loading]
# ─────────────────────────────────────────────────────────────
def clean_year_data_to_hourly(df: pd.DataFrame, year: int) -> pd.DataFrame:
    ...
    # Preserve whether each hourly temperature is supported by an original
    # ISD observation. This is recorded before the existing limit=2
    # interpolation, so interpolated values never count as raw support.
    hourly["temp_obs_count"] = (
        obs_hourly["obs_count"]
        .reindex(full_idx)
        .fillna(0)
        .astype("int16")
    )
    ...
    invalid_day_mask = valid_hour_count < 8
    hourly.loc[invalid_day_mask, ["temp_C", "dewpoint_C"]] = np.nan

    # Observations from a day rejected by the existing daily gate must not
    # subsequently count as valid raw support for a period-specific FFT.
    hourly.loc[invalid_day_mask, "temp_obs_count"] = 0
    hourly["temp_is_raw_observed"] = (
        hourly["temp_obs_count"] > 0
    ).astype("int8")
...
# ─────────────────────────────────────────────────────────────
# 7b. Absolute-threshold heatwave detection.
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# 8. FFT & Diurnal Metrics  [dew-point diurnal outputs]
# ─────────────────────────────────────────────────────────────
...
def _finite_pair_diff(urban_value, rural_value):
    """
    Return urban minus rural only when both values are finite.

    If either side is missing or non-finite, return NaN.
    """
    try:
        urban_value = float(urban_value)
        rural_value = float(rural_value)
    except (TypeError, ValueError):
        return np.nan

    if (
        np.isfinite(urban_value)
        and np.isfinite(rural_value)
    ):
...
def compute_fft(df_local: pd.DataFrame, n_harm=N_HARMONICS):
    """
    Compute period-specific two-harmonic diurnal cycles.

    The main temperature FFT is unchanged. For WBGT/labour only,
    temperature and dew point are restricted to identical valid timestamps
    within each station-period. Both common-time curves are reconstructed
    with n_harm harmonics (N_HARMONICS=2: 24 h + 12 h).
    """
    ...
    required_qc_columns = {
        "local_date",
        "local_hour",
        "temp_is_raw_observed",
    }
    missing_qc_columns = sorted(
        required_qc_columns.difference(df_local.columns)
    )
    if missing_qc_columns:
        raise KeyError(
            "compute_fft missing raw-support QC columns: "
            f"{missing_qc_columns}"
        )

    # Count all local dates selected for this station-period before rows with
    # missing temperature are removed. For HW/NHW inputs, this denominator is
    # therefore the number of contributing event/non-event local dates.
    n_period_dates = int(df_local["local_date"].nunique())

    # Main temperature analysis is otherwise unchanged.
    ...
    # This pre-fill curve may contain the existing limit=2 short-gap
    # interpolation, but has not received the final cross-bin interpolation.
    hourly_t = tmp.groupby("h_bin")["temp_C"].mean().reindex(range(24))
    supported_hour_bin_count = int(hourly_t.notna().sum())

    raw_mask = (
        pd.to_numeric(tmp["temp_is_raw_observed"], errors="coerce")
        .fillna(0)
        .eq(1)
    )
    raw_night_mask = raw_mask & tmp["h_bin"].isin(TEMP_QC_NIGHT_HOURS)
    raw_complement_mask = (
        raw_mask & tmp["h_bin"].isin(TEMP_QC_COMPLEMENT_HOURS)
    )

    raw_hour_bin_count = int(tmp.loc[raw_mask, "h_bin"].nunique())
    raw_night_hour_bin_count = int(
        tmp.loc[raw_night_mask, "h_bin"].nunique()
    )
    raw_complement_hour_bin_count = int(
        tmp.loc[raw_complement_mask, "h_bin"].nunique()
    )
    raw_night_day_count = int(
        tmp.loc[raw_night_mask, "local_date"].nunique()
    )
    raw_complement_day_count = int(
        tmp.loc[raw_complement_mask, "local_date"].nunique()
    )

    if n_period_dates <= 0:
        return None

    raw_night_day_frac = float(raw_night_day_count / n_period_dates)
    raw_complement_day_frac = float(
        raw_complement_day_count / n_period_dates
    )

    temperature_diurnal_qc_pass = bool(
        supported_hour_bin_count >= TEMP_QC_MIN_SUPPORTED_HOUR_BINS
        and raw_night_hour_bin_count
        >= TEMP_QC_MIN_RAW_HOUR_BINS_PER_WINDOW
        and raw_complement_hour_bin_count
        >= TEMP_QC_MIN_RAW_HOUR_BINS_PER_WINDOW
        and raw_night_day_frac >= TEMP_QC_MIN_RAW_WINDOW_DAY_FRAC
        and raw_complement_day_frac >= TEMP_QC_MIN_RAW_WINDOW_DAY_FRAC
    )
    if not temperature_diurnal_qc_pass:
        return None

    climatology_fill_hour_count = int(24 - supported_hour_bin_count)
    climatology_fill_frac = float(climatology_fill_hour_count / 24.0)
    ...
    for k in range(1, n_harm + 1):
        amps.append(float(2.0 * np.abs(coef[k])))
        phases.append(float(np.angle(coef[k])))

    reconstructed = reconstruct_diurnal(mean_t, amps, phases)

    # Backward-compatible dew-point climatology.
    dew_curve = np.full(24, np.nan, dtype=float)

    # WBGT-only T and Td curves built from identical timestamps.
    wbgt_temp_fft_curve = np.full(24, np.nan, dtype=float)
    dew_fft_curve = np.full(24, np.nan, dtype=float)

    dew_mean = np.nan
    dew_amps = [np.nan for _ in range(n_harm)]
    dew_phases = [np.nan for _ in range(n_harm)]
    dew_tmax_fft = np.nan
    dew_tmin_fft = np.nan

    common_time_count = 0
    temp_valid_time_count = int(len(tmp))
    common_time_frac = 0.0
    common_hour_bin_count = 0
    common_hour_bin_frac = 0.0
    common_fft_available = 0

    if "dewpoint_C" in tmp.columns:
        # Preserve the old hourly dew-point climatology fields.
        hourly_d_legacy = (
            tmp.groupby("h_bin")["dewpoint_C"]
            .mean()
            .reindex(range(24))
        )
        if hourly_d_legacy.notna().sum() >= 3:
            hourly_d_legacy = (
                hourly_d_legacy
                .interpolate(method="linear", limit_direction="both")
                .ffill()
        ...
        dew_curve = hourly_d_legacy.to_numpy(dtype=float)

        # Exact station-level common timestamps for WBGT T and Td.
        common = tmp[["h_bin", "temp_C", "dewpoint_C"]].copy()
        common["temp_C"] = pd.to_numeric(common["temp_C"], errors="coerce")
        common["dewpoint_C"] = pd.to_numeric(
            common["dewpoint_C"], errors="coerce"
        )
        common = (
            common
            .replace([np.inf, -np.inf], np.nan)
            .dropna(subset=["temp_C", "dewpoint_C"])
        )

        common_time_count = int(len(common))
        if temp_valid_time_count > 0:
            common_time_frac = float(
                common_time_count / temp_valid_time_count
            )

        hourly_common = (
            common.groupby("h_bin")[["temp_C", "dewpoint_C"]]
            .mean()
            .reindex(range(24))
        )

        common_hour_bin_count = int(
            hourly_common[["temp_C", "dewpoint_C"]]
            .notna()
            .all(axis=1)
            .sum()
        )
        common_hour_bin_frac = float(common_hour_bin_count / 24.0)

        if common_hour_bin_count >= 8:
            hourly_common = (
                hourly_common
                .interpolate(method="linear", limit_direction="both")
                .ffill()
        ...
        common_values = hourly_common[["temp_C", "dewpoint_C"]].to_numpy(
            dtype=float
        )

        if (
            common_time_frac >= DEWPOINT_MIN_VALID_FRAC
            and common_hour_bin_count >= 8
            and np.isfinite(common_values).all()
        ):
            # Common-time temperature: two harmonics.
            wbgt_temp_values = hourly_common["temp_C"].to_numpy(dtype=float)
            wbgt_temp_coef = sp_fft(wbgt_temp_values) / len(wbgt_temp_values)
            wbgt_temp_mean = float(np.real(wbgt_temp_coef[0]))
            wbgt_temp_amps, wbgt_temp_phases = [], []
            for k in range(1, n_harm + 1):
                wbgt_temp_amps.append(
                    float(2.0 * np.abs(wbgt_temp_coef[k]))
                )
                wbgt_temp_phases.append(
                    float(np.angle(wbgt_temp_coef[k]))
                )
            wbgt_temp_fft_curve = reconstruct_diurnal(
                wbgt_temp_mean,
                wbgt_temp_amps,
                wbgt_temp_phases,
            )

            # Common-time dew point: the same two harmonics.
            dew_values = hourly_common["dewpoint_C"].to_numpy(dtype=float)
            dew_coef = sp_fft(dew_values) / len(dew_values)
            dew_mean = float(np.real(dew_coef[0]))
            dew_amps, dew_phases = [], []
            for k in range(1, n_harm + 1):
                dew_amps.append(float(2.0 * np.abs(dew_coef[k])))
                dew_phases.append(float(np.angle(dew_coef[k])))

            dew_fft_curve = reconstruct_diurnal(
                dew_mean,
                dew_amps,
                dew_phases,
            )
            dew_tmax_fft = float(np.max(dew_fft_curve))
            dew_tmin_fft = float(np.min(dew_fft_curve))
            common_fft_available = 1
    ...
    return dict(
        ...
        hourly_dew_fft=dew_fft_curve,
        dew_mean=dew_mean,
        dew_amplitudes=dew_amps,
        dew_phases=dew_phases,
        dew_Tmax_fft=dew_tmax_fft,
        dew_Tmin_fft=dew_tmin_fft,

        # New temperature curve paired with dew point for WBGT/labour.
        hourly_wbgt_temp_fft=wbgt_temp_fft_curve,

        n_days=int(tmp["local_date"].nunique()),

        # Minimal two-window raw temperature-support QC. These diagnostics
        # describe support before the final 24-bin climatological filling.
        temperature_diurnal_qc_version=TEMP_DIURNAL_QC_VERSION,
        temperature_diurnal_qc_pass=int(temperature_diurnal_qc_pass),
        temperature_period_date_count=int(n_period_dates),
        temperature_supported_hour_bin_count=int(
            supported_hour_bin_count
        ),
        temperature_climatology_fill_hour_count=int(
            climatology_fill_hour_count
        ),
        temperature_climatology_fill_frac=float(
            climatology_fill_frac
        ),
        temperature_raw_hour_bin_count=int(raw_hour_bin_count),
        temperature_raw_night_hour_bin_count=int(
            raw_night_hour_bin_count
        ),
        temperature_raw_complement_hour_bin_count=int(
            raw_complement_hour_bin_count
        ),
        temperature_raw_night_day_count=int(raw_night_day_count),
        temperature_raw_complement_day_count=int(
            raw_complement_day_count
        ),
        temperature_raw_night_day_frac=float(raw_night_day_frac),
        temperature_raw_complement_day_frac=float(
            raw_complement_day_frac
        ),

        # Existing QC field names retained; they now describe exact
        # common-time T/Td support before climatology filling and FFT.
        dewpoint_valid_hour_count=common_hour_bin_count,
        dewpoint_valid_frac=common_hour_bin_frac,
        dewpoint_valid_time_count=common_time_count,
        dewpoint_total_time_count=temp_valid_time_count,
        dewpoint_valid_time_frac=common_time_frac,
        dewpoint_fft_available=int(common_fft_available),

        # Explicit aliases for downstream clarity.
        temp_dew_common_time_count=common_time_count,
        temp_valid_time_count_for_wbgt=temp_valid_time_count,
        temp_dew_common_time_frac=common_time_frac,
        temp_dew_common_hour_bin_count=common_hour_bin_count,
        temp_dew_common_hour_bin_frac=common_hour_bin_frac,
        temp_dew_common_fft_available=int(common_fft_available),
    )
...
def save_mechanism_regressions(all_df, output_dir):
    """
    In addition to annual/heatwave/non_heatwave/warm_season subsets,
    ...
    # Add KG main-group stratified subsets.
...
def save_mediation_analysis(all_df, output_dir):
    """
    Adds annual_kg_A/B/C/D/E strata to the structured subsets,
...
def save_kggroup_robustness(all_df, output_dir):
    """
    Summarize robustness by Koppen-Geiger main group (A/B/C/D/E).
...
def build_period_records(
    ...
    for period_name, (sub_u, sub_r) in periods.items():
        ...
        # Dew-point FFT values for this pair-period.
        u_dew_mean = fft_u.get(
            "dew_mean",
            np.nan,
        )

        r_dew_mean = fft_r.get(
            "dew_mean",
            np.nan,
        )

        u_dew_tmax = fft_u.get(
            "dew_Tmax_fft",
            np.nan,
        )

        r_dew_tmax = fft_r.get(
            "dew_Tmax_fft",
            np.nan,
        )

        u_dew_tmin = fft_u.get(
            "dew_Tmin_fft",
            np.nan,
        )

        r_dew_tmin = fft_r.get(
            "dew_Tmin_fft",
            np.nan,
        )

        u_dew_amps = fft_u.get(
            "dew_amplitudes",
            [np.nan] * N_HARMONICS,
        )

        r_dew_amps = fft_r.get(
            "dew_amplitudes",
            [np.nan] * N_HARMONICS,
        )

        u_dew_phases = fft_u.get(
            "dew_phases",
            [np.nan] * N_HARMONICS,
        )

        r_dew_phases = fft_r.get(
            "dew_phases",
            [np.nan] * N_HARMONICS,
        )
        ...
        rec = {
            ...
            # Method-protocol fields for provenance.
            ...
            # ── Dew-point FFT results ──────────────────────────
            "urban_dew_Tmean": u_dew_mean,
            "rural_dew_Tmean": r_dew_mean,

            "dDewTmean": _finite_pair_diff(
                u_dew_mean,
                r_dew_mean,
            ),

            "urban_dew_Tmax_fft": u_dew_tmax,
            "rural_dew_Tmax_fft": r_dew_tmax,

            "dDewTx": _finite_pair_diff(
                u_dew_tmax,
                r_dew_tmax,
            ),

            "urban_dew_Tmin_fft": u_dew_tmin,
            "rural_dew_Tmin_fft": r_dew_tmin,

            "dDewTn": _finite_pair_diff(
                u_dew_tmin,
                r_dew_tmin,
            ),
        }
        ...
        for k in range(N_HARMONICS):
            ...
            u_dew_phase = (
                u_dew_phases[k]
                if k < len(u_dew_phases)
                else np.nan
            )

            r_dew_phase = (
                r_dew_phases[k]
                if k < len(r_dew_phases)
                else np.nan
            )
            ...
            rec[
                f"urban_dew_Phase{k + 1}"
            ] = u_dew_phase

            rec[
                f"rural_dew_Phase{k + 1}"
            ] = r_dew_phase
        ...
        # New FFT-reconstructed dew-point curves and paired differences.
        for h in range(24):
            u_dew_fft = (
                fft_u["hourly_dew_fft"][h]
                if fft_u.get("hourly_dew_fft") is not None
                else np.nan
            )

            r_dew_fft = (
                fft_r["hourly_dew_fft"][h]
                if fft_r.get("hourly_dew_fft") is not None
                else np.nan
            )

            rec[
                f"urban_dew_fft_h{h:02d}"
            ] = (
                float(u_dew_fft)
                if np.isfinite(u_dew_fft)
                else np.nan
            )

            rec[
                f"rural_dew_fft_h{h:02d}"
            ] = (
                float(r_dew_fft)
                if np.isfinite(r_dew_fft)
                else np.nan
            )
        ...
        # Common-time temperature FFT used only for WBGT/labour.
        u_wbgt_temp_curve = fft_u.get(
            "hourly_wbgt_temp_fft",
            np.full(24, np.nan, dtype=float),
        )
        r_wbgt_temp_curve = fft_r.get(
            "hourly_wbgt_temp_fft",
            np.full(24, np.nan, dtype=float),
        )

        for h in range(24):
            rec[f"urban_wbgt_temp_fft_h{h:02d}"] = (
                float(u_wbgt_temp_curve[h])
                if np.isfinite(u_wbgt_temp_curve[h])
                else np.nan
            )
            rec[f"rural_wbgt_temp_fft_h{h:02d}"] = (
                float(r_wbgt_temp_curve[h])
                if np.isfinite(r_wbgt_temp_curve[h])
                else np.nan
            )
        # Dewpoint QC diagnostics (for WBGT/labour filtering only;
        # do NOT use for temperature-based main analysis filtering)
        # Dew-point QC diagnostics.
        # The 80% rule uses actual valid timestamps within the period,
        # not the number of filled hourly climatology bins.
        dew_u_valid_bins = int(
            fft_u.get(
                "dewpoint_valid_hour_count",
                0,
            )
        )

        dew_r_valid_bins = int(
            fft_r.get(
                "dewpoint_valid_hour_count",
                0,
            )
        ...
        dew_u_time_count = int(
            fft_u.get(
                "dewpoint_valid_time_count",
                0,
            )
        )

        dew_r_time_count = int(
            fft_r.get(
                "dewpoint_valid_time_count",
                0,
            )
        ...
        dew_u_total_count = int(
            fft_u.get(
                "dewpoint_total_time_count",
                0,
            )
        )

        dew_r_total_count = int(
            fft_r.get(
                "dewpoint_total_time_count",
                0,
            )
        ...
        dew_u_time_frac = float(
            fft_u.get(
                "dewpoint_valid_time_frac",
                0.0,
            )
        )

        dew_r_time_frac = float(
            fft_r.get(
                "dewpoint_valid_time_frac",
                0.0,
            )
        ...
        dew_u_fft_ok = (
            int(
                fft_u.get(
                    "dewpoint_fft_available",
                    0,
                )
            ) == 1
            and dew_u_time_frac
            >= DEWPOINT_MIN_VALID_FRAC
        )
        ...
        rec["dewpoint_valid_hour_count_urban"] = (
            dew_u_valid_bins
        )

        rec["dewpoint_valid_hour_count_rural"] = (
            dew_r_valid_bins
        )

        rec["dewpoint_valid_frac_urban"] = (
            fft_u.get(
                "dewpoint_valid_frac",
                0.0,
            )
        ...
        rec["dewpoint_valid_time_count_urban"] = (
            dew_u_time_count
        )

        rec["dewpoint_valid_time_count_rural"] = (
            dew_r_time_count
        )

        rec["dewpoint_total_time_count_urban"] = (
            dew_u_total_count
        )

        rec["dewpoint_total_time_count_rural"] = (
            dew_r_total_count
        )

        rec["dewpoint_valid_time_frac_urban"] = (
            dew_u_time_frac
        )

        rec["dewpoint_valid_time_frac_rural"] = (
            dew_r_time_frac
        )

        rec["dewpoint_fft_available_urban"] = int(
            dew_u_fft_ok
        )

        rec["dewpoint_fft_available_rural"] = int(
            dew_r_fft_ok
        )

        # Explicit common-time aliases for labour/WBGT.
        rec["temp_dew_common_time_frac_urban"] = dew_u_time_frac
        rec["temp_dew_common_time_frac_rural"] = dew_r_time_frac
        rec["temp_dew_common_fft_available_urban"] = int(dew_u_fft_ok)
        rec["temp_dew_common_fft_available_rural"] = int(dew_r_fft_ok)

        # Both stations must independently pass the 80% rule.
        rec["has_dewpoint_for_wbgt"] = int(
            dew_u_fft_ok
            and dew_r_fft_ok
        )

        rec["wbgt_qc_pass"] = int(
            rec["has_dewpoint_for_wbgt"]
        )

        # Temperature-only two-window support diagnostics. These fields are
        # descriptive for retained station-periods and do not alter any
        # downstream temperature, dew-point, WBGT or FFT calculation.
        integer_temperature_qc_fields = (
            "temperature_diurnal_qc_pass",
            "temperature_period_date_count",
            "temperature_supported_hour_bin_count",
            "temperature_climatology_fill_hour_count",
            "temperature_raw_hour_bin_count",
            "temperature_raw_night_hour_bin_count",
            "temperature_raw_complement_hour_bin_count",
            "temperature_raw_night_day_count",
            "temperature_raw_complement_day_count",
        )
        float_temperature_qc_fields = (
            "temperature_climatology_fill_frac",
            "temperature_raw_night_day_frac",
            "temperature_raw_complement_day_frac",
        )

        for station_label, station_fft in (
            ("urban", fft_u),
            ("rural", fft_r),
        ):
            for field in integer_temperature_qc_fields:
                rec[f"{field}_{station_label}"] = int(
                    station_fft.get(field, 0)
                )
            for field in float_temperature_qc_fields:
                rec[f"{field}_{station_label}"] = float(
                    station_fft.get(field, np.nan)
                )

        rec["temperature_diurnal_qc_pass_pair"] = int(
            fft_u.get("temperature_diurnal_qc_pass", 0) == 1
            and fft_r.get("temperature_diurnal_qc_pass", 0) == 1
        )
        rec["temperature_diurnal_qc_version"] = (
            TEMP_DIURNAL_QC_VERSION
        )
...
def process_single_pair(args):
    ...
    # Extract KG climate-zone information.
    try:
    ...
    # Add threshold-reference metadata to station_threshold_summary.
    ...
    if not records_out:
        error_info = {
            ...
            "missing_data": (
                "station-period failed raw two-window temperature-support "
                "QC, pre-fill hourly-bin QC, or FFT reconstruction"
            )
        }
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compute pair-level urban-rural heatwave response metrics.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--pair-csv", default=PAIR_CSV_PATH, help="Station-pair metadata table.")
    parser.add_argument("--station-meta", default=STATION_META_PATH, help="Station-level auxiliary metadata table.")
    parser.add_argument("--pair-fallback", default=None, help="Fallback pair-level auxiliary table.")
    parser.add_argument("--isd-base-dir", default=ISD_BASE_DIR, help="Root directory of ISD-Lite yearly files.")
    parser.add_argument("--era5-station-dir", default=ERA5_STATION_DIR, help="Directory of station daily Tmax CSV files.")
    parser.add_argument("--continent-shp", default=CONTINENT_SHP, help="Continent polygon shapefile.")
    parser.add_argument("--building-volume-csv", default=BV_CSV_PATH, help="Station building-volume table.")
    parser.add_argument("--kg-tif", default=KG_TIF, help="Koppen-Geiger raster.")
    parser.add_argument("--output-dir", default=OUTPUT_DIR, help="Directory for analysis outputs.")
    parser.add_argument("--disable-era5-climatology", action="store_true", help="Use the ISD-only heatwave-threshold fallback.")
    return parser.parse_args()


def apply_cli_args(args):
    global PAIR_CSV_PATH, STATION_META_PATH, PAIR_FALLBACK_PATH, ISD_BASE_DIR
    global ERA5_STATION_DIR, CONTINENT_SHP, BV_CSV_PATH, KG_TIF, OUTPUT_DIR
    global USE_ERA5_CLIMATOLOGY
    PAIR_CSV_PATH = args.pair_csv
    STATION_META_PATH = args.station_meta
    PAIR_FALLBACK_PATH = args.pair_fallback or args.pair_csv
    ISD_BASE_DIR = args.isd_base_dir
    ERA5_STATION_DIR = args.era5_station_dir
    CONTINENT_SHP = args.continent_shp
    BV_CSV_PATH = args.building_volume_csv
    KG_TIF = args.kg_tif
    OUTPUT_DIR = args.output_dir
    if args.disable_era5_climatology:
        USE_ERA5_CLIMATOLOGY = False
...
def main():
    ...
    print(
        " Temperature QC: raw observations required on ≥"
        f"{TEMP_QC_MIN_RAW_WINDOW_DAY_FRAC:.0%} of contributing local dates "
        "in both 00:00–07:59 and 08:00–23:59 windows; "
        f"pre-fill supported bins ≥{TEMP_QC_MIN_SUPPORTED_HOUR_BINS} "
        f"[{TEMP_DIURNAL_QC_VERSION}]"
    )
    ...
    with ProcessPoolExecutor(max_workers=n_cores, initializer=apply_cli_args, initargs=(parse_args(),)) as executor:
    ...
    for method in ["percentile", "absolute"]:
        ...
        save_kggroup_robustness(sub_m, method_dir)      #  新增
    ...
    # ── 合并对比文件（：含 kg_code / kg_group）────────────────
    compare_cols = [
        "pair_id", "group", "continent", "lat_group",
        "kg_code", "kg_group", "climate_zone_main",   #  新增
        "hw_method", "hw_abs_threshold",
        "hne_ref_mode", "hw_ref_mode",                #  新增
        "dTmean", "dAmp1", "dTx", "dTn",
        "dT_day_mean", "dT_night_mean",
        "delta_tropical_night_freq", "delta_hotnight_excess",
    ...
    # ── 阈值诊断表（：含 hw_ref_mode / kg 字段，已在子进程写入）
    if threshold_rows:
    ...
    check_cols = [
        ...
        "kg_code", "kg_group", "climate_zone_main",   #  新增检查
        "hne_ref_mode", "hw_ref_mode",                 #  新增检查
    ]
    for c in check_cols:
    ...
    print(
        ...
        "  │   ├── kggroup_robustness_summary.csv  ← KG groups A-E and detailed codes\n"
