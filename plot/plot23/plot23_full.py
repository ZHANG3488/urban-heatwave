...
# ─────────────────────────────────────────────────────────────
# ── 改为 ──
INPUT_DIR  = str(_OPEN_SOURCE_PATHS.main_multiyear_percentile_dir)
OUTPUT_DIR = str(_OPEN_SOURCE_PATHS.fig23_output)
    ...
    for region in region_order:
        ...
        if len(g) == 0:
            region_rows.append({
                ...
                "n_UCI_to_UCI": 0,
                "n_UCI_to_UHI": 0,
                "n_UHI_strengthened": 0,
                "n_UHI_weakened": 0,
                "n_UCI_strengthened": 0,
                "n_UCI_weakened": 0,
                "n_region_switch": 0,
                "n_other": 0,
            })
        ...
        region_rows.append({
            ...
            "n_UHI_to_UHI": int((g["tx_transition_4class"] == "UHI_to_UHI").sum()),
            "n_UHI_to_UCI": int((g["tx_transition_4class"] == "UHI_to_UCI").sum()),
            "n_UCI_to_UCI": int((g["tx_transition_4class"] == "UCI_to_UCI").sum()),
            "n_UCI_to_UHI": int((g["tx_transition_4class"] == "UCI_to_UHI").sum()),
            "n_UHI_strengthened": int((g["uhi_strength_change"] == "UHI_strengthened").sum()),
            "n_UHI_weakened": int((g["uhi_strength_change"] == "UHI_weakened").sum()),
            "n_UCI_strengthened": int((g["uci_strength_change"] == "UCI_strengthened").sum()),
            "n_UCI_weakened": int((g["uci_strength_change"] == "UCI_weakened").sum()),
        ...
        for _, r in region_summary.iterrows():
            ...
            if r["n"] <= 0:
                ...
                txt = (
                    ...
                    + f"UHI→UHI {int(r['n_UHI_to_UHI'])} | UHI→UCI {int(r['n_UHI_to_UCI'])}\n"
                    + f"UCI→UCI {int(r['n_UCI_to_UCI'])} | UCI→UHI {int(r['n_UCI_to_UHI'])}\n"
                    + f"UHI↑ {int(r['n_UHI_strengthened'])} | UHI↓ {int(r['n_UHI_weakened'])}\n"
                    + f"UCI↑ {int(r['n_UCI_strengthened'])} | UCI↓ {int(r['n_UCI_weakened'])}"
                )
...
def plot_figure_hw_shift_arrows(all_df, output_dir):
    ...
    Panel b:
        ...
        Separate NHW-UHI-like and NHW-UCI-like subsets.
        Categories:
...
# ─────────────────────────────────────────────────────────────
# [NEW] Figure 4: Human Impacts (Exposure-based proxies)
# ─────────────────────────────────────────────────────────────
def plot_figure4_human_impacts(all_df, output_dir):
    ...
    with plt.rc_context({
        ...
        fig.tight_layout()
        custom_out_dir = str(_OPEN_SOURCE_PATHS.figures_dir)
...
def plot_si_annual_nhw_hw_state_transition(all_df, output_dir):
    """
    Additional SI figure/table: annual → NHW → HW period-specific state changes.

    Uses the same percentile + matched HW/NHW base cohort as the mechanism figures.
    The four sign-defined transitions are retained; zero/unclassified cases are
    reported in tables and annotations rather than converted into a fifth class.
    """
    if all_df is None or len(all_df) == 0:
    ...
    for suffix, label in zip(period_order, period_labels):
        ...
        for state in ["UHI-like", "UCI-like"]:
    ...
    with plt.rc_context({
        ...
        class_labels_short = {
            "UHI-like -> UHI-like": "UHI→UHI",
            "UHI-like -> UCI-like": "UHI→UCI",
            "UCI-like -> UCI-like": "UCI→UCI",
            "UCI-like -> UHI-like": "UCI→UHI",
        }
        class_colors = {
            "UHI-like -> UHI-like": "#b3000d",
            "UHI-like -> UCI-like": "#4292c6",
            "UCI-like -> UCI-like": "#08519c",
            "UCI-like -> UHI-like": "#ef3b2c",
        }
        ...
        fig.text(0.080, 0.045,
                 ...
                 f"Zero/unclassified sign cases are reported in CSV, not plotted as a fifth transition class.",
...
def main():
    ...
    print("\n--- [新增] Figure 4: Human Impacts (Exposure-based proxies) ---")
    plot_figure4_human_impacts(all_df, OUTPUT_DIR)   # <------ 【在这里添加这一行】
