...
[v4-allstation patch]
- Diurnal profiles, map metric and all boxplots now read from the upstream
  heatwave_detection_all_stations output (station_diurnal_reconstructed.csv),
  combining urban and rural stations without distinction.
- Pairs with no detected heatwave show delta_Tmax = 0 on the map.
- Subplot titles for CDH / HDH / Sleep loss / Economic loss no longer carry
  the "(urban)" qualifier; all stations are pooled.
"""
...
MORTALITY_DIR = UNIFIED_ROOT + "/analysis/mortality_heri"
...
# ── [v4-allstation] upstream heatwave detection output ───────────────────────
HW_DETECTION_DIR = UNIFIED_ROOT + "/analysis/heatwave_flags"
...
def derive_dunne_tx_labour_loss(df_lc):
    """Add positive urban-minus-rural Dunne labour loss at station Tx."""
    ...
    if missing:
        raise ValueError(
            "Cannot derive Dunne Tx labour loss; missing columns: "
            f"{missing[:20]}" + (f" ... total={len(missing)}" if len(missing) > 20 else "")
        )
    def _curve(row, prefix):
        return np.array([row.get(f"{prefix}{h:02d}", np.nan) for h in range(24)], dtype=float)
    def _capacity(w):
        if not np.isfinite(w): return np.nan
        return float(np.clip(100.0 - 25.0 * max(float(w)-25.0, 0.0)**(2.0/3.0), 0.0, 100.0))
    def _one(row):
        tu = _curve(row, "urban_diurnal_h"); tr = _curve(row, "rural_diurnal_h")
        wu = _curve(row, "wbgt_urban_h"); wr = _curve(row, "wbgt_rural_h")
        if not np.isfinite(tu).any() or not np.isfinite(tr).any():
            return pd.Series({"d_labour_loss_tx": np.nan})
        hu = int(np.nanargmax(tu)); hr = int(np.nanargmax(tr))
        w_u = wu[hu] if np.isfinite(wu[hu]) else np.nan
        w_r = wr[hr] if np.isfinite(wr[hr]) else np.nan
        lc_u = _capacity(w_u); lc_r = _capacity(w_r)
        loss_u = 100.0-lc_u if np.isfinite(lc_u) else np.nan
        loss_r = 100.0-lc_r if np.isfinite(lc_r) else np.nan
        return pd.Series({
            "tx_hour_urban": hu, "tx_hour_rural": hr,
            "labour_loss_tx_u": loss_u, "labour_loss_tx_r": loss_r,
            "d_labour_loss_tx": loss_u-loss_r if np.isfinite(loss_u) and np.isfinite(loss_r) else np.nan,
        })
    d = out.apply(_one, axis=1)
    for c in d.columns:
        out[c] = d[c].to_numpy()
    return out
...
def load_mortality_rr(diag):
    p = os.path.join(MORTALITY_DIR, "rr_01_pair_period.csv")
    if not os.path.exists(p):
        diag.add("Load mortality", f"missing: {p}")
        return None
    df = pd.read_csv(p)
    df["period_norm"] = df["period"].apply(norm_period)
    diag.add("Load mortality", f"path={p}\nshape={df.shape}")
    return df
...
def build_station_period_from_mortality(df_mort, diag):
    if df_mort is None or len(df_mort) == 0:
        return pd.DataFrame()

    keep_periods = {"HW", "NHW", "JJA"}
    if "period_norm" in df_mort.columns:
        df_mort = df_mort[df_mort["period_norm"].isin(keep_periods)].copy()
    ...
    needed_cols = ["pair_id", "period_norm", "rr_urban", "rr_rural"]
    needed_cols = [c for c in needed_cols if c in df_mort.columns]
    df_mort = df_mort[needed_cols].copy()

    df = stationize_pair_period(df_mort, metric_specs)
    if len(df) == 0:
        return df
    ...
    diag.add("Build station-period from mortality", f"shape={df.shape}")
    return df

def merge_station_modules(df_phys, df_hne_st, df_mort_st, df_lab_st, diag):
    frames = [x for x in [df_phys, df_hne_st, df_mort_st, df_lab_st]
...
def build_station_climate_map_from_hw_diurnal(df_hw_diurnal):
    ...
    station_diurnal_reconstructed.csv.
    ...
    # the upstream CSV, but drop_duplicates just in case.
...
def build_diurnal_long_from_hw_diurnal(df_hw_diurnal, diag):
    ...
    station_diurnal_reconstructed.csv.  Urban and rural stations are
...
def build_hw_nhw_tmax_map_metric_from_hw_diurnal(df_hw_diurnal, diag):
    ...
    # Delta UHI (HW - NHW)
    merged["delta_uhi_hw_nhw"] = (merged["uhi_HW"] - merged["uhi_NHW"]).fillna(0)
...
def make_composite_maps_de_figure_v2(df_hw_diurnal, df_fig9, out_dir, diag, fig_name="composite_delta_uhi_final"):
    ...
    draw_side_latitudinal_profile(ax_a_prof, prof_day, "#d73027", "$\Delta$T (°C)")
    ...
    draw_side_latitudinal_profile(ax_b_prof, prof_night, "#4575b4", "$\Delta$T (°C)")
...
def make_composite_maps_de_figure_v3(df_hw_diurnal, df_fig9, out_dir, diag, fig_name="composite_delta_uhi_final"):
    ...
    draw_side_latitudinal_profile(ax_a_prof, prof_day, "#d73027", "$\Delta$Tx (°C)")
    ax_a_prof.set_yticklabels([])
    ...
    draw_side_latitudinal_profile(ax_b_prof, prof_night, "#4575b4", "$\Delta$Tn (°C)")
    ax_b_prof.set_yticklabels([])
...
def make_latitudinal_analysis_figure(df_hw_diurnal, out_dir, diag):
    ...
    cb.set_label("$\Delta$UHI (°C)", fontsize=6)
...
def build_n_days_table(df_hw_diurnal, diag):
    """
    从上游 station_diurnal_reconstructed.csv 提取每站点每时期的
    ...
    # JJA fallback: use standard 92 days if no data matched
    jja_mask = (df["period_norm"] == "JJA") & df["n_days_per_year"].isna()
    df.loc[jja_mask, "n_days_per_year"] = 92.0
...
def main():
    ...
    print("  · 5-row layout: map | diurnal | RR+productivity | CDH+HDH | sleep+econ")
    print("  · map & diurnal profiles sourced from upstream HW detection CSV")
    ...
    if df_mort is not None: df_mort = df_mort[df_mort["pair_id"].astype(str).isin(matched_ids)].copy()
    ...
    df_mort_st = build_station_period_from_mortality(df_mort, diag)
    df_lab_st  = build_station_period_from_labour(df_lc, diag)

    print("\n[5/7] Merging station-period modules...")
    df_station = merge_station_modules(df_phys, df_hne_st, df_mort_st, df_lab_st, diag)

    print("\n[6/7] Merging climate zone labels + building diurnal summary...")
    # ── [v4-allstation] Climate labels from upstream HW diurnal CSV ───────────
    ...
    # ── [v4-allstation] Diurnal long table from upstream CSV ──────────────────
