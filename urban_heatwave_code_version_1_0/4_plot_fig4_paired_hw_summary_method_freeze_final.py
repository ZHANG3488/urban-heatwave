...
Negative values indicate greater rural marginal sensitivity.
    ...
    labour loss: Dunne model at station-specific work-hour Tx shaded WBGT derived from common-time two-harmonic T/Td curves
...
# Independent common-cohort variant.
# The actual number of pairs is read dynamically from
...
FILES: Dict[str, str] = {
    ...
    "amplification": "main_hw_amplification_paired_summary.csv",
}
...
def write_provenance_manifest(
    ...
    manifest = {
        ...
        "plotting_notes": [
            ...
            "Panel c uses direct outcome-specific temporal burden components, not regression-derived contributions.",
            "Sleep panel-c rows use direct urban-rural sleep-loss differences with day=0 and night=total.",
            ...
            "Labour-loss rows use the Dunne model at station-specific work-hour Tx shaded WBGT derived from common-time two-harmonic T/Td curves.",
            ...
            "The HW amplification text file reads the upstream matched pair-level contrast R_p=(U-R)_HW-(U-R)_NHW.",
            "Mean amplification is reported separately for UHI and UCI pairs, together with their difference; no regression model is fitted.",
        ],
    }
...
def render_combined_main_figure(
    ...
    # 绘制各 panel
    # Save map statistics beside the corresponding original/common figure,
    # rather than writing one shared file to the current working directory.
    map_stats_path = safe_output_path(
        out_dir
        / f"{basename}_map_statistics.txt"
    )
    ...
    draw_map_combined(
        ...
        a,
        "a",
        stats_output_path=map_stats_path,
    )

    draw_sensitivity_combined(
        axb,
        b,
        "b",
    )
    ...
    if c is None or len(c) == 0:
        ...
        if not present:
            ...
            axc_uhi.set_xlabel("Scaled urban–rural burden component", fontsize=7.3)
            axc_uci.set_xlabel("Scaled urban–rural burden component", fontsize=7.3)
            ...
            fig.text(
                title_x,
                title_y,
                "Outcome-specific day–night components\nof modelled urban–rural burden",
                ha="center", va="bottom",
                fontsize=10,
                fontweight="bold",
            )
...
def export_uhi_uci_detailed_summary_txt2(b_data: pd.DataFrame, d_data: pd.DataFrame, supp_data: pd.DataFrame, out_dir: Path, basename: str):
    ...
    # --- Part 2: auxiliary panel-b sensitivity / direct burden summary ---
    lines.append("\n" + "-" * 50)
...
def export_uhi_uci_detailed_summary_txt(
    ...
    lines.append("  labour_loss uses the Dunne model at station-specific work-hour Tx shaded WBGT derived from common-time two-harmonic T/Td curves.")
...
def export_hw_amplification_stats_txt(
    ...
    """Export matched UHI/UCI amplification means, differences and percentages.

    The upstream table contains the pair-level estimand
        R_p = (Y_urban-Y_rural)_HW - (Y_urban-Y_rural)_NHW.

    This function reports mean R_p separately for UHI and UCI pairs and their
    difference. It does not fit or report a regression model. The percentage
    change is descriptive and uses matched group means:
        100 * (HW mean - NHW mean) / abs(NHW mean).
    """
    out_path = safe_output_path(
        out_dir / f"{basename}_hw_amplification_growth.txt"
    )
    ...
    if amplification_data is None or len(amplification_data) == 0:
        raise ValueError("Paired HW amplification summary table is missing or empty.")
    missing = required - set(amplification_data.columns)
    if missing:
        raise ValueError(
            "Paired HW amplification summary table missing columns: "
            f"{sorted(missing)}"
        )

    data = amplification_data.copy()
    data = data[data["component"].astype(str).str.lower().eq("total")].copy()

    def _format_matched_mean_change(nhw: float, hw: float) -> Tuple[str, str]:
        """Return descriptive group-mean percentage change and status."""
        if not np.isfinite(nhw) or not np.isfinite(hw):
            return "undefined", "non-finite mean"
        if np.isclose(nhw, 0.0, atol=1e-12):
            return "undefined", "NHW mean = 0"
        pct = 100.0 * (hw - nhw) / abs(nhw)
        sign_reversal = (
            not np.isclose(hw, 0.0, atol=1e-12)
            and np.sign(hw) != np.sign(nhw)
        )
        return f"{pct:+.2f}%", ("sign reversal" if sign_reversal else "same sign")

    lines: List[str] = []
    lines.append("PAIRED HW AMPLIFICATION SUMMARY")
    lines.append("=" * 170)
    lines.append("Pair-level burden contrast:")
    lines.append("  DeltaY_p,q = (Y_urban - Y_rural)_p,q")
    lines.append("  R_p = DeltaY_p,HW - DeltaY_p,NHW")
    lines.append("")
    lines.append("Reported summaries:")
    lines.append("  UHI: mean R_p among annual-UHI pairs")
    lines.append("  UCI: mean R_p among annual-UCI pairs")
    lines.append("  UCI-UHI: difference between the two group means")
    lines.append("  No regression model is fitted.")
    lines.append("")
    lines.append("Matched-mean percentage change reported for UHI and UCI groups:")
    lines.append("  Change (%) = 100 x (matched HW mean - matched NHW mean) / |matched NHW mean|")
    lines.append("  This percentage is descriptive only; it is not the mean of pair-level percentages")
    lines.append("  and does not replace R_p or its station-pair bootstrap confidence interval.")
    lines.append("  Percentages with a sign reversal are retained but explicitly flagged.")
    lines.append("")
    lines.append(
        "Inference: UHI/UCI-stratified station-pair bootstrap; each selected "
        "pair retains its matched NHW and HW values."
    )
    lines.append("Interpretation: modelled paired burden contrasts, not causal effects.")
    lines.append("")

    order = ["sleep", "building_energy", "labour_loss"]
    for outcome in order:
        sub = data[data["outcome"].astype(str).eq(outcome)].copy()
        if sub.empty:
            continue

        unit = str(sub["unit"].iloc[0]) if "unit" in sub.columns else ""
        lines.append("-" * 170)
        lines.append(f"Outcome: {outcome_display_label(outcome)}  |  Unit: {unit}")
        lines.append("-" * 170)
        lines.append(
            f"  {'Summary':22s} | {'Matched NHW':>13s} | {'Matched HW':>13s} | "
            f"{'Mean R = HW-NHW':>15s} | {'Change vs NHW':>15s} | "
            f"{'Percentage status':>19s} | {'95% CI':>27s} | {'n pairs':>8s}"
        )
        lines.append("  " + "-" * 166)

        for group in ["UHI", "UCI", "UCI_minus_UHI"]:
            row = sub[sub["group"].astype(str).eq(group)]
            if row.empty:
                continue
            r = row.iloc[0]
            label = {
                "UHI": "UHI mean",
                "UCI": "UCI mean",
                "UCI_minus_UHI": "UCI-UHI difference",
            }[group]
            ci = f"[{float(r['ci_low']):.4f}, {float(r['ci_high']):.4f}]"

            if group in {"UHI", "UCI"}:
                pct_text, pct_status = _format_matched_mean_change(
                    float(r["matched_nhw_mean"]),
                    float(r["matched_hw_mean"]),
                )
            else:
                pct_text, pct_status = "N/A", "between-group difference"

            lines.append(
                f"  {label:22s} | "
                f"{float(r['matched_nhw_mean']):13.4f} | "
                f"{float(r['matched_hw_mean']):13.4f} | "
                f"{float(r['estimate']):15.4f} | "
                f"{pct_text:>15s} | "
                f"{pct_status:>24s} | "
                f"{ci:>27s} | "
                f"{int(round(float(r['n_pairs']))):8d}"
            )
        lines.append("")

    out_path.write_text("\n".join(lines), encoding="utf-8")
    print(f"[export] paired HW amplification summary saved to: {out_path}")
    return out_path

def validate_matched_labour_phase_counts(
    b_data: Optional[pd.DataFrame],
    c_data: Optional[pd.DataFrame],
) -> None:
    """
    Validate that labour-loss NHW and HW rows were produced from the
    same number of station pairs within each UHI/UCI group.

    This function does not perform matching. Pair-level matching must
    already have been completed in the preparation script.
    """
    ...
    def _validate_one_table(
        ...
        if data is None or len(data) == 0:
            raise ValueError(
                f"{table_name}: input table is missing or empty."
            )
        ...
        if total_only:
            if "component" not in work.columns:
                raise ValueError(
                    f"{table_name}: component column is missing."
                )
            ...
            work = work[
                work["component"].eq("total")
            ].copy()

        work["n_clusters"] = pd.to_numeric(
            work["n_clusters"],
            errors="coerce",
        )
        ...
        for group in UHI_UCI_GROUPS:
            ...
            for phase in ("NHW", "HW"):
                ...
                if len(values) != 1:
                    raise ValueError(
                        f"{table_name}: expected one labour "
                        f"n_clusters value for {group}-{phase}, "
            ...
            if counts["NHW"] != counts["HW"]:
                raise ValueError(
                    f"{table_name}: unmatched labour sample "
                    f"for {group}: "
                    f"NHW={counts['NHW']}, "
                    f"HW={counts['HW']}."
                )
            ...
            print(
                f"[validate] {table_name} {group}: "
                f"NHW = HW = {counts['NHW']} pairs"
            )

    _validate_one_table(
        b_data,
        table_name="Panel b labour",
        total_only=False,
    )

    _validate_one_table(
        c_data,
        table_name="Panel c labour",
        total_only=True,
    )

def export_labour_valid_counts_txt(
    b_data: Optional[pd.DataFrame],
    c_data: Optional[pd.DataFrame],
    out_dir: Path,
) -> Path:
    """
    Export valid labour-loss station-pair counts by UHI/UCI group and phase.

    Count source:
        n_clusters is preferred because pair_id is the bootstrap cluster.
        n_obs is used only as a backward-compatible fallback.

    Validity rules:
        Panel b:
            outcome == labour_loss
            panel-b estimate is finite
    ...
    This function only reports counts. It does not modify plotted values.
    """
    out_path = out_dir / "labour_valid_pair_counts.txt"

    def _extract_panel_counts(
        data: Optional[pd.DataFrame],
        *,
        panel_name: str,
    ) -> Tuple[
        ...
        if data is None or len(data) == 0:
            notes.append(
                f"{panel_name}: input table is missing or empty."
            )
            return counts, notes
        ...
        if missing:
            notes.append(
                f"{panel_name}: missing required columns "
                f"{sorted(missing)}."
            )
            return counts, notes
        ...
        work = work[
            ...
            & work["uhi_uci_group"].isin(
                UHI_UCI_GROUPS
            )
            & work["period_phase"].isin(
                ["NHW", "HW"]
            )
        ].copy()

        if panel_name == "Panel b":
            estimate_col, _, _ = (
                _panel_b_numeric_columns(work)
            )

            if estimate_col not in work.columns:
                notes.append(
                    f"{panel_name}: no supported estimate "
                    "column was found."
                )
                return counts, notes

            work["_valid_estimate"] = pd.to_numeric(
                work[estimate_col],
                errors="coerce",
            )
            ...
            if not required_c.issubset(work.columns):
                notes.append(
                    f"{panel_name}: component/estimate "
                    "columns are missing."
                )
                return counts, notes
        ...
        work = work[
            np.isfinite(work["_valid_estimate"])
        ].copy()

        count_col = None

        for candidate in ("n_clusters", "n_obs"):
        ...
        if count_col is None:
            notes.append(
                f"{panel_name}: neither n_clusters nor "
                "n_obs is available; valid pair counts "
                "cannot be recovered."
            )
            return counts, notes

        work["_valid_pair_count"] = pd.to_numeric(
            work[count_col],
            errors="coerce",
        )
        ...
        for group in UHI_UCI_GROUPS:
            for phase in ("NHW", "HW"):
                ...
                if len(values) == 0:
                    counts[(group, phase)] = None

                elif len(values) == 1:
                    counts[(group, phase)] = int(
                        round(float(values[0]))
                    )

                else:
                    raise ValueError(
                        f"{panel_name}: inconsistent "
                        "valid-pair counts for "
                        f"{group}-{phase}: "
                        f"{sorted(values.tolist())}"
                    )

        notes.append(
            f"{panel_name}: counts read from "
            f"'{count_col}' after requiring a "
            "finite labour-loss estimate."
        )

        return counts, notes

    panel_b_counts, panel_b_notes = (
        _extract_panel_counts(
            b_data,
            panel_name="Panel b",
        )
    )

    panel_c_counts, panel_c_notes = (
        _extract_panel_counts(
            c_data,
            panel_name="Panel c",
        )
    ...
    lines: List[str] = [
        "=" * 72,
        "VALID LABOUR-LOSS STATION-PAIR COUNTS",
        "=" * 72,
        (
            "Generated: "
            f"{pd.Timestamp.now():%Y-%m-%d %H:%M:%S}"
        ),
        "",
        "Definition:",
        (
            "  Valid means the prepared labour-loss "
            "estimate is finite after upstream "
            "missing-value filtering."
        ),
        (
            "  Missing dew point, WBGT or labour "
            "records are not counted as zero."
        ),
        "",
        (
        ...
        (
            "A cross-phase common-pair count cannot "
            "be reconstructed from aggregated "
            "plotting tables."
        ),
        "",
        "PANEL B: LABOUR-LOSS MARGINAL SENSITIVITY",
        "-" * 72,
    ]

    for group in UHI_UCI_GROUPS:
        nhw = panel_b_counts[(group, "NHW")]
        hw = panel_b_counts[(group, "HW")]

        lines.append(
            f"  {group}: "
            f"NHW = {nhw if nhw is not None else 'N/A'} pairs; "
            f"HW = {hw if hw is not None else 'N/A'} pairs"
        )

    lines.extend([
        "",
        (
            "PANEL C: LABOUR-LOSS BURDEN "
            "(TOTAL = DAY; NIGHT STRUCTURAL ZERO)"
        ),
        "-" * 72,
    ])

    for group in UHI_UCI_GROUPS:
        nhw = panel_c_counts[(group, "NHW")]
        hw = panel_c_counts[(group, "HW")]
    ...
    print(
        "[export] Labour valid-pair counts "
        f"saved to: {out_path}"
    )
...
# =============================================================================
# Labour-common combined figure
# =============================================================================

def _validate_labour_common_plot_inputs(
    ...
    """
    Validate the independently prepared labour-common Figure 4 inputs.

    Requirements
    ------------
    1. Panel a must contain exactly the same pair IDs as the dynamically
       detected labour-common cohort.
    2. Panel-a UHI/UCI labels must match the cohort lookup.
    3. Every panel-b and panel-c row must report the expected number of
       pairs for its UHI/UCI group.
    4. No fixed total pair count is assumed.
    """
    if cohort is None or cohort.empty:
        raise ValueError(
            "labour_common_pair_ids.csv is empty."
        )
    ...
    missing = required_cohort - set(
        cohort.columns
    )

    if missing:
        raise ValueError(
            "labour_common_pair_ids.csv is missing columns: "
            f"{sorted(missing)}"
        )
    ...
    cohort_work = cohort_work[
        cohort_work["uhi_uci_group"].isin(
            UHI_UCI_GROUPS
        )
    ].copy()
    ...
    duplicate_ids = (
        cohort_work.loc[
            cohort_work["pair_id"].duplicated(
                keep=False
            ),
            "pair_id",
        ]
    ...
    if duplicate_ids:
        raise ValueError(
            "labour_common_pair_ids.csv contains duplicate pair IDs. "
    ...
    # -----------------------------------------------------------------
    # Panel a: exact pair IDs and group labels
    # -----------------------------------------------------------------
    required_a = {
        "pair_id",
        "uhi_uci_group",
    ...
    missing_a = required_a - set(
        panel_a.columns
    )

    if missing_a:
        raise ValueError(
            "Labour-common panel a is missing columns: "
            f"{sorted(missing_a)}"
        )
    ...
    if panel_a_ids != expected_ids:
        ...
        raise ValueError(
            "Labour-common panel a does not use the exact cohort: "
    ...
    group_mismatch = group_check[
        group_check["uhi_uci_group_panel_a"]
        != group_check["uhi_uci_group_cohort"]
    ]

    if not group_mismatch.empty:
        raise ValueError(
            "Panel-a UHI/UCI labels differ from the labour-common lookup. "
            f"Examples: "
            f"{group_mismatch['pair_id'].head(20).tolist()}"
        )

    # -----------------------------------------------------------------
    # Panels b/c: all outcome rows must retain the group-specific cohort
    # -----------------------------------------------------------------
    ...
    for table_name, table in [
        ...
        missing_table = required - set(
            table.columns
        )

        if missing_table:
            raise ValueError(
                f"Labour-common {table_name} is missing columns: "
                f"{sorted(missing_table)}"
            )
        ...
        for _, row in work.iterrows():
            ...
            if not np.isfinite(observed):
                count_errors.append(
                    ...
                    f"{row.get('outcome')} / "
                    f"{group} / "
                    f"{row.get('period_phase')} "
                    "has invalid n_clusters"
                )
                continue

            observed_n = int(
                round(float(observed))
            )
            ...
            if observed_n != expected_n:
                count_errors.append(
                    ...
                    f"{group} / "
                    f"{row.get('period_phase')} / "
                    f"{row.get('component', 'not_applicable')}: "
    ...
    if count_errors:
        raise ValueError(
            "Some labour-common panel rows lost pairs:\n"
            + "\n".join(
                f"  - {msg}"
    ...
    return {
        ...
        "group_counts": expected_by_group,
        "pair_ids": expected_ids,
    }


def render_labour_common_combined_result(
    standard_data_dir: Path,
    standard_out_dir: Path,
    basename: str,
) -> List[Path]:
    """
    Read the independently prepared labour-common a/b/c files and render
    one additional combined Figure 4.

    The original full-sample data and original combined figure are not
    ...
    # Missing common files should not prevent the original full-sample
    # figures from being generated.
    if missing_paths:
        print(
            "[skip] Labour-common combined figure was not generated "
            "because prepared common-cohort files are missing:",
            file=sys.stderr,
        )
    ...
    if any(
        ...
        raise ValueError(
            "One or more labour-common input files could not be read."
        )

    b_common = harmonize_model_outcomes(
        b_common,
        "labour-common panel b",
    )

    c_common = harmonize_model_outcomes(
        c_common,
        "labour-common panel c",
    )

    # Existing validation: labour NHW/HW counts must match.
    validate_matched_labour_phase_counts(
        b_common,
        c_common,
    )

    # New strict validation: every outcome must use the dynamically
    # detected labour-common sample.
    validation = _validate_labour_common_plot_inputs(
        panel_a=a_common,
        panel_b=b_common,
        panel_c=c_common,
        cohort=cohort,
    ...
    print(
        f"  input directory : {common_data_dir}"
    )
    print(
        f"  output directory: {common_out_dir}"
    )
    print(
        f"  actual pairs    : {validation['n_pairs']}"
    )
    print(
        f"  group counts    : {validation['group_counts']}"
    )

    outputs = render_combined_main_figure(
        a=a_common,
        b=b_common,
        c=c_common,
    ...
    # Independent count report for the common-cohort version.
    common_count_path = export_labour_valid_counts_txt(
        b_common,
        c_common,
        common_out_dir,
    )
    ...
    metadata_path = safe_output_path(
        common_out_dir
        / f"{basename}_labour_common_metadata.json"
    )

    metadata = {
        "variant": "labour_common",
        "created_at_utc": utc_now_iso(),
        "pair_count_mode": "runtime_dynamic",
        "actual_n_pairs": validation["n_pairs"],
        "group_counts": validation["group_counts"],
        "full_sample_outputs_modified": False,
        "input_directory": str(
            common_data_dir
        ),
...
def main() -> int:
    ...
    ap.add_argument(
        "--skip-labour-common-combined",
        action="store_true",
        help=(
            "Skip the additional combined figure generated from the "
            "dynamically detected labour-common cohort."
        ),
    )
    ...
    path_amplification = data_dir / FILES["amplification"]

    a = load_csv(path_a)
    b = load_csv(path_b)
    c = load_csv(path_c)
    supp = load_csv(path_supp)
    amplification = load_csv(path_amplification)
    ...
    csv_audits = [
        ...
        dataframe_audit(
            ...
            path_amplification,
            amplification,
            REQUIRED_COLUMNS["amplification"],
        ),
    ]
    ...
    labour_count_path = export_labour_valid_counts_txt(
        b,
        c,
        out_dir,
    )
    ...
    # Additional combined result using the exact dynamically detected
    # labour-common cohort. The original full-sample result above is retained.
    if not args.skip_labour_common_combined:
        labour_common_outputs = (
            render_labour_common_combined_result(
                standard_data_dir=data_dir,
                standard_out_dir=out_dir,
                basename=args.combined_basename,
            )
        )
    ...
    # Export the matched pair-level HW amplification summary.
    amplification_txt = export_hw_amplification_stats_txt(
        amplification,
        out_dir,
        args.combined_basename,
    )
    output_paths.append(amplification_txt)
