...
# §0. 全局配置
# ══════════════════════════════════════════════════════════════
UNIFIED_ROOT = str(_OPEN_SOURCE_PATHS.unified_root)
PAIR_CSV_PATH   = str(_OPEN_SOURCE_PATHS.pair_csv)
ISD_BASE_DIR    = str(_OPEN_SOURCE_PATHS.isd_dir)
OUTPUT_DIR      = str(_OPEN_SOURCE_PATHS.hne_output)
CANONICAL_GROUP_CSV = (
...
STATION_FEATURES_CSV = str(_OPEN_SOURCE_PATHS.station_features_csv)
...
def compute_sleep_loss_minutes(
    ...
    """
    分段线性样条睡眠损失模型。
    来源：Minor et al. (2022) One Earth, Table S37
    返回：ΔSleep (min/night)，负值=减少，0=无效应
    """
    tmin = np.asarray(tmin, dtype=float)
    return (const
            + beta1 * np.maximum(tmin - knot1, 0.0)
            + beta2 * np.maximum(tmin - knot2, 0.0))
...
def build_station_panel(df_all, df_warm, lat, station_label,
    ...
    # [ERA5-MOD start] no ISD fallback when ERA5 is enabled
    if USE_ERA5_CLIMATOLOGY:
        _era5_tmax = (load_era5_tmax_series(era5_usaf, era5_wban, era5_label)
                      if era5_usaf else None)
        if _era5_tmax is None:
            print(f"    [SKIP-{station_label}] ERA5 Tmax missing/invalid")
            return {
                "pooled": {"panel": None, "hw_flag": pd.Series(dtype=int)},
                "loyo":   {"panel": None, "hw_flag": pd.Series(dtype=int)},
            }
        _doy_thr = compute_hw_doy_thr_era5(_era5_tmax)
        hw_thr_series_raw = apply_doy_thr_to_index(tmax_all.index, _doy_thr)

        if USE_QM_BIAS_CORRECTION:
            hw_thr_series, qm_diagnostics = apply_quantile_mapping_bias_correction(
                hw_thr_series_raw=hw_thr_series_raw,
                isd_tmax=tmax_all,
                era5_tmax=_era5_tmax,
            ...
            raise NotImplementedError("Monthly mean bias correction is not implemented in this script.")
        else:
            hw_thr_series = hw_thr_series_raw.copy()
            qm_diagnostics = {}

    else:
        hw_thr_series, _ = compute_hw_threshold_from_tmax(tmax_all)
    # [ERA5-MOD end]
...
def build_paired_output(pair_id, row,
    ...
    if USE_ERA5_CLIMATOLOGY:
        ...
        if _era5_tmax_u is None:
            print("  [SKIP-paired] urban ERA5 Tmax missing/invalid")
            return [], []
        hw_thr_u_raw = apply_doy_thr_to_index(
            tmax_u_all.index,
            compute_hw_doy_thr_era5(_era5_tmax_u)
        )

        if USE_QM_BIAS_CORRECTION:
            hw_thr_u, qm_diagnostics = apply_quantile_mapping_bias_correction(
                hw_thr_series_raw=hw_thr_u_raw,
                isd_tmax=tmax_u_all,
        ...
        hw_thr_u, _ = compute_hw_threshold_from_tmax(tmax_u_all)

    _ext_pair_hw = align_analysis_hw_flags(
        analysis_hw_flags, tmax_u_all.index
    )
    required_flag_cols = [
        "is_warm_season",
        "hw_flag_percentile_warm_season",
    ...
    if (
        ...
        print(
            f"  [SKIP-paired] {pair_id}: canonical HW/NHW flags "
            "do not align; fallback disabled"
        )
        return [], []

    valid_pair_flags = (
        _ext_pair_hw[required_flag_cols].notna().all(axis=1)
    )
    hw_flag_u = pd.Series(
        np.nan, index=tmax_u_all.index, name="hw_flag"
    )
    hw_flag_u.loc[valid_pair_flags] = (
        _ext_pair_hw.loc[
            valid_pair_flags, "hw_flag_percentile_warm_season"
        ].astype(int).values
    )
    hw_source = "analysis_multiyear_daily_heatwave_flags"
    # [ERA5-MOD end]
...
def export_fig123_data_sleep(all_paired_warm, out_dir):
    if not all_paired_warm:
        print("  [Fig1-3] No paired warm season data found for sleep loss. Skipping.")
        return

    df = pd.concat(all_paired_warm, ignore_index=True)

    # 1. Use the canonical manuscript-level UHI/UCI grouping.
    # Do NOT redefine UHI/UCI here using local dTMAX.
    # dTMAX_pair_mean is retained only as a diagnostic if dTMAX exists.
    ...
    # 避免沿用旧的逐日动态 group 或 module-local group
    if "group" in df.columns:
        df = df.drop(columns=["group"])

    df["pair_id"] = df["pair_id"].astype(str)
    df = df.merge(canonical_group, on="pair_id", how="left")
    df = df.merge(pair_diag, on="pair_id", how="left")

    missing_group = df["group"].isna()
    if missing_group.any():
        missing_pairs = sorted(
            df.loc[missing_group, "pair_id"].astype(str).unique()
        )[:20]
        raise ValueError(
            "Missing canonical UHI/UCI group for export_fig123_data_sleep; "
    ...
    # === 图 1：日循环真实数据：24 小时温度 -> hourly sleep loss + CI ===
    # compute_sleep_loss_minutes() 返回的是 ΔSleep，负值=睡眠减少。
    # 为了让输出更直观，这里转换为：
    #   sleep_loss_amount = -compute_sleep_loss_minutes(temp)
    # 因此：
    #   d_sleep_loss_min_hourly_mean > 0 表示 Urban 比 Rural 睡眠损失更多
    #   d_sleep_loss_min_hourly_mean < 0 表示 Urban 比 Rural 睡眠损失更少
    fig1_rows = []

    temp_u_cols = [f"temp_h{h:02d}_U" for h in range(24)]
    temp_r_cols = [f"temp_h{h:02d}_R" for h in range(24)]

    missing = [c for c in temp_u_cols + temp_r_cols if c not in sub.columns]
    if missing:
        raise ValueError(
            "Cannot export hourly sleep loss: missing hourly temperature columns. "
    ...
    for (grp, prd), g in sub.groupby(["group", "period"]):
        for h in range(24):
            ...
            u_sleep_raw = compute_sleep_loss_minutes(g[u_col].values)
            r_sleep_raw = compute_sleep_loss_minutes(g[r_col].values)
            ...
            # Urban - Rural；正值表示城市额外睡眠损失
            vals = u_loss - r_loss
            vals = pd.Series(vals).replace([np.inf, -np.inf], np.nan).dropna().values.astype(float)
            ...
            # bootstrap 95% CI；复用本脚本已有 bootstrap_ci()
            ...
            fig1_rows.append({
                ...
                "hour": h,
                "n": int(len(vals)),
                "d_sleep_loss_min_hourly_mean": mean_val,
                "d_sleep_loss_min_hourly_se": se_val,
                "d_sleep_loss_min_hourly_ci_lo": ci_lo,
                "d_sleep_loss_min_hourly_ci_hi": ci_hi,
            })

    pd.DataFrame(fig1_rows).to_csv(
        os.path.join(fig_data_dir, "fig1_diurnal_sleep_real.csv"),
        index=False
    )


    # === 图 2：脆弱性差距真实数据 (绝对值) ===
    sub["sleep_loss_min_U_pos"] = -sub["sleep_loss_min_U"]
    sub["sleep_loss_min_R_pos"] = -sub["sleep_loss_min_R"]

    fig2 = sub.groupby(["group", "period"])[
        ["sleep_loss_min_U_pos", "sleep_loss_min_R_pos"]
    ].mean().reset_index()
    ...
    fig2.to_csv(
        os.path.join(fig_data_dir, "fig2_vulnerability_sleep_real.csv"),
        index=False
    )

    # === 图 3：机制散点真实数据 (配对差值) ===
    # 原始 d_sleep_loss_min = sleep_loss_min_U - sleep_loss_min_R
    # 但 sleep_loss_min 本身是负值=睡眠减少
    # 因此这里转成：
    #   d_sleep_loss_min_positive > 0 表示 Urban 比 Rural 睡眠损失更多
    fig3 = sub[["pair_id", "group", "period", "dTMIN", "d_sleep_loss_min"]].copy()

    # 原始 d_sleep_loss_min = sleep_loss_min_U - sleep_loss_min_R
    # sleep_loss_min 本身负值=睡眠减少
    # 转成正值=Urban 比 Rural 睡眠损失更多
    fig3["d_sleep_loss_min"] = -fig3["d_sleep_loss_min"]
    ...
    fig3.to_csv(
        os.path.join(fig_data_dir, "fig3_mechanism_sleep_real.csv"),
        index=False
    )

    print(f"  [Fig1-3] Sleep loss real data exported to: {fig_data_dir}")
...
def _fig4_compute_daily_damp_from_hourly(df):
    """
    Compute daily urban-rural diurnal amplitude contrast from hourly temperatures.

    dAmp1_daily = (Tmax_U - Tmin_U) - (Tmax_R - Tmin_R)

    This gives a Figure-4-local amplitude variable when dAmp1 is not available
    from the main mechanism pipeline.
    """
    u_cols = [f"temp_h{h:02d}_U" for h in range(24)]
    r_cols = [f"temp_h{h:02d}_R" for h in range(24)]

    if any(c not in df.columns for c in u_cols + r_cols):
        return np.full(len(df), np.nan, dtype=float)

    u_arr = df[u_cols].apply(pd.to_numeric, errors="coerce").values.astype(float)
    r_arr = df[r_cols].apply(pd.to_numeric, errors="coerce").values.astype(float)

    u_max = _fig4_safe_nanmax(u_arr)
    u_min = _fig4_safe_nanmin(u_arr)
    r_max = _fig4_safe_nanmax(r_arr)
    r_min = _fig4_safe_nanmin(r_arr)

    return (u_max - u_min) - (r_max - r_min)
...
def _fig4_build_sleep_pair_period_table(
    ...
    """
    Build Figure 4 sleep pair-period table.

    Input:
        all_paired_warm:
            list of paired daily panels from build_paired_output(),
            including warm_season / heatwave / non_heatwave periods.
    ...
    Key definitions:
        night_heat_exposure:
            ...
            Σnight[max(T_U - theta_night, 0) - max(T_R - theta_night, 0)]

        night_hne_exposure:
            original HNE exposure contrast:
            hne_d_U - hne_d_R = dHNE

        sleep_heat_burden:
            urban extra sleep loss relative to rural.
            Positive means urban sleep loss is larger than rural sleep loss.

            Because sleep_loss_min is negative when sleep is reduced:
                sleep_heat_burden = -(sleep_loss_min_U - sleep_loss_min_R)
                                  = -d_sleep_loss_min
    """
    if not all_paired_warm:
        raise ValueError("[Fig4 sleep] all_paired_warm is empty.")

    df = pd.concat(all_paired_warm, ignore_index=True).copy()

    if "hne_method" in df.columns:
        df = df[df["hne_method"] == primary_hne_method].copy()

    df = df[df["period"].isin(["heatwave", "non_heatwave"])].copy()
    ...
    if "pair_id" not in df.columns:
        raise ValueError("[Fig4 sleep] Missing required column: pair_id")

    df["period_norm"] = df["period"].apply(_fig4_period_norm_from_period)
    df = _fig4_assign_pair_group(df)

    # ── Fixed-hour dry-bulb exposure: night and day ─────────────
    nhe_u, n_night_u = _fig4_exposure_from_hourly_cols(
        df, "U", FIG4_NIGHT_HOURS, theta_night
    )
    nhe_r, n_night_r = _fig4_exposure_from_hourly_cols(
        df, "R", FIG4_NIGHT_HOURS, theta_night
    )

    dhe_u, n_day_u = _fig4_exposure_from_hourly_cols(
        df, "U", FIG4_DAY_HOURS, theta_day
    )
    dhe_r, n_day_r = _fig4_exposure_from_hourly_cols(
        df, "R", FIG4_DAY_HOURS, theta_day
    )

    df["night_heat_exposure_urban"] = nhe_u
    df["night_heat_exposure_rural"] = nhe_r
    df["night_heat_exposure"] = df["night_heat_exposure_urban"] - df["night_heat_exposure_rural"]

    df["day_heat_exposure_urban"] = dhe_u
    df["day_heat_exposure_rural"] = dhe_r
    df["day_heat_exposure"] = df["day_heat_exposure_urban"] - df["day_heat_exposure_rural"]
    ...
    # ── HNE exposure from original solar-night model ────────────
    if "hne_d_U" in df.columns:
        df["night_hne_exposure_urban"] = df["hne_d_U"]
    else:
        df["night_hne_exposure_urban"] = np.nan

    if "hne_d_R" in df.columns:
        df["night_hne_exposure_rural"] = df["hne_d_R"]
    else:
        df["night_hne_exposure_rural"] = np.nan

    if "dHNE" in df.columns:
        df["night_hne_exposure"] = df["dHNE"]
    else:
        df["night_hne_exposure"] = (
            df["night_hne_exposure_urban"] - df["night_hne_exposure_rural"]
        )

    # ── Daily dAmp1 from hourly temperature curves ──────────────
    df["dAmp1"] = _fig4_compute_daily_damp_from_hourly(df)

    # ── Sleep-loss direction cleanup ────────────────────────────
    # Original sleep_loss_min: negative = sleep reduced.
    if "sleep_loss_min_U" in df.columns:
        df["sleep_loss_amount_U"] = -pd.to_numeric(df["sleep_loss_min_U"], errors="coerce")
    else:
        df["sleep_loss_amount_U"] = np.nan

    if "sleep_loss_min_R" in df.columns:
        df["sleep_loss_amount_R"] = -pd.to_numeric(df["sleep_loss_min_R"], errors="coerce")
    else:
        df["sleep_loss_amount_R"] = np.nan

    if "d_sleep_loss_min" in df.columns:
        df["sleep_heat_burden"] = -pd.to_numeric(df["d_sleep_loss_min"], errors="coerce")
    else:
        df["sleep_heat_burden"] = (
            df["sleep_loss_amount_U"] - df["sleep_loss_amount_R"]
        )

    # Positive sleep_heat_burden means:
    # urban sleep loss amount is larger than rural sleep loss amount.

    # Sleep-related productivity loss, optional.
    if "d_econ_loss_pct_short" in df.columns:
        df["sleep_econ_loss_pct_short"] = df["d_econ_loss_pct_short"]
    else:
        df["sleep_econ_loss_pct_short"] = np.nan

    if "d_econ_loss_usd_sleep" in df.columns:
        df["sleep_econ_loss_usd"] = df["d_econ_loss_usd_sleep"]
    else:
        df["sleep_econ_loss_usd"] = np.nan

    # ── Aggregate to pair-period level ──────────────────────────
    meta_cols = [
        "group", "dTMAX_pair_mean",
        "kg_group", "kg_code", "climate_zone_main",
        "lat_group", "continent",
        "lat_urban", "lon_urban",
        "hne_method", "output_type",
    ]
    ...
    if "date" in out.columns:
        out = out.rename(columns={"date": "n_days"})

    out["theta_night_temp"] = theta_night
    out["theta_day_temp"] = theta_day
    out["fig4_night_hours"] = ",".join([str(h) for h in FIG4_NIGHT_HOURS])
    out["fig4_day_hours"] = ",".join([str(h) for h in FIG4_DAY_HOURS])
    out["primary_hne_method"] = primary_hne_method

    front_cols = [
        "pair_id", "period_norm", "group",
        "dAmp1",
        "night_heat_exposure",
        "day_heat_exposure",
        "night_hne_exposure",
...
def _fig4_make_sleep_paired_diffs(pair_period_df):
    ...
    for c in [
        "group", "dTMAX_pair_mean",
        "kg_group", "kg_code", "climate_zone_main",
        "lat_group", "continent", "lat_urban", "lon_urban"
    ]:
        c_hw = f"{c}_HW"
    ...
    # Amplitude-damping regime, sleep-local version.
...
def _fig4_write_sleep_schema(out_dir):
    schema_path = os.path.join(out_dir, "fig4_sleep_schema.txt")
    ...
    lines = [
        ...
        "      computed from local hourly temperature curves temp_h00...temp_h23",
        "      unit: degC",
        "",
