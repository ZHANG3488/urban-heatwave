...
OUTPUT_DIR = str(_OPEN_SOURCE_PATHS.cdh_output)
...
def process_single_pair(args):
    ...
    if len(common_dates) > 0:
        delta_tmax_ann = float((u_tmax[common_dates] - r_tmax[common_dates]).mean())
        group = "UHI" if delta_tmax_ann > 0 else "UCI"
    else:
        result["error"] = (
            "no_common_tmax_dates_for_uhi_uci_grouping; "
            "pair skipped instead of assigning default UHI"
        )
        return result
    ...
    # [ERA5-MOD start]
    # 必须使用 ERA5 长气候态计算热浪 DOY 阈值。
    # ERA5 文件缺失、列名不对或数据不足时，直接跳过该 pair。
    # 不再回退到 ISD P90，也不再调用 compute_hw_threshold_from_tmax()。
    ...
    try:
        _doy_thr = compute_hw_doy_thr_era5(_era5_tmax_u)
        hw_thr_series_raw = apply_doy_thr_to_index(u_tmax.index, _doy_thr)

        if hw_thr_series_raw.isna().all():
            result["error"] = "ERA5_missing_or_invalid"
            return result
        ...
        if hw_thr_series.isna().all():
            result["error"] = "ERA5_QM_threshold_invalid"
            return result
    ...
    # Canonical HW/NHW dates must come from analysis_multiyear.py.
    analysis_hw_flags = get_analysis_hw_flags_for_pair(pid)
    _aligned_hw = align_analysis_hw_flags(
        analysis_hw_flags, u_tmax.index
    )
    required_flag_cols = [
        "is_warm_season",
        "hw_flag_percentile_warm_season",
    ...
    if (
        ...
        result["error"] = (
            "canonical analysis_multiyear HW/NHW flags missing or "
    ...
    hw_mask.loc[valid_hw] = (
        _aligned_hw.loc[
            valid_hw, "hw_flag_percentile_warm_season"
