...
Methods:
  ...
  WBGT (shade) = 0.7 * WBT + 0.3 * T_drybulb
  WBT: Stull (2011) approximation from T and dewpoint Td
"""
...
def shade_wbgt(T_C, Td_C):
    ...
    Td = np.asarray(Td_C, float)
    wbgt = np.full_like(T, np.nan)
    valid = np.isfinite(T) & np.isfinite(Td)
    if valid.any():
        rh_v = rh_from_t_td(T[valid], Td[valid])
        wbgt[valid] = 0.7 * wbt_stull(T[valid], rh_v) + 0.3 * T[valid]
    missing = np.isfinite(T) & ~np.isfinite(Td)
    if missing.any():
        rh_fb = np.full(missing.sum(), 60.0)
        wbgt[missing] = 0.7 * wbt_stull(T[missing], rh_fb) + 0.3 * T[missing]
    return wbgt
...
def extract_T(row, role):
    return _get_curve(row, f"{role}_diurnal", "_h{h:02d}")

def extract_Td(row, role):
    return _get_curve(row, f"{role}", "_dew_h{h:02d}")
...
def compute_row_lc(row):
    T_u  = extract_T(row, "urban");  T_r  = extract_T(row, "rural")
    Td_u = extract_Td(row, "urban"); Td_r = extract_Td(row, "rural")

    has_dew = (np.isfinite(Td_u).sum() >= 12) and (np.isfinite(Td_r).sum() >= 12)
    wbgt_u  = shade_wbgt(T_u, Td_u)
    wbgt_r  = shade_wbgt(T_r, Td_r)
    out     = {"has_dewpoint": int(has_dew)}
    ...
    for method, dlc in methods.items():
        ...
        # ------------------------------------------------------------
        # ★ 新增：基于大气温度峰值的 Labour Loss 差值计算 (Peak-to-Peak)
        # ------------------------------------------------------------
        # 1. 确定白天工作时段的索引
        work_idx = np.array(WORK_HOURS) 
        
        # 2. 提取该时段内城市和农村的大气温度最大值 (Tmax)
        u_tmax = np.nanmax(T_u[work_idx]) if np.any(np.isfinite(T_u[work_idx])) else np.nan
        r_tmax = np.nanmax(T_r[work_idx]) if np.any(np.isfinite(T_r[work_idx])) else np.nan

        if pd.notna(u_tmax) and pd.notna(r_tmax):
            # 3. 根据当前 method 对应的公式计算最高温时刻的损失
            # 损失定义 = 100 - 劳动能力(LC)
            if method == "dunne":
                u_peak_loss = 100.0 - lc_dunne(u_tmax)
                r_peak_loss = 100.0 - lc_dunne(r_tmax)
            else:
                # 对应 He (2022) 的 high, medium, low 强度
                u_peak_loss = 100.0 - workability_he(u_tmax, intensity=method)
                r_peak_loss = 100.0 - workability_he(r_tmax, intensity=method)
            
            # 4. 计算差值：城市最高温损失 - 农村最高温损失
            # 如果城市更凉快 (UCI)，u_peak_loss < r_peak_loss，结果为负值
            out[f"dloss_{method}_peak_t_diff"] = float(u_peak_loss - r_peak_loss)
        else:
            out[f"dloss_{method}_peak_t_diff"] = np.nan
...
def main():
    ...
    if "has_dewpoint" in df.columns:
        ...
        print(f"    Dewpoint coverage: {cov:.1%}  (RH=60% fallback for {1-cov:.1%})")
