...
Output directory:
  <legacy_result_root>/
  sensitivity_analysis/data_quality_sensitivity/sensitivity_analysis_v7
"""
...
# Sensitivity output directory requested by the user.
OUTPUT_DIR = str(_OPEN_SOURCE_PATHS.sensitivity_output)
...
def prepare_heatwave_mask_and_threshold(
    ...
    Returns
    -------
    hw_mask_all, hw_thr_series_for_intensity, threshold_info, error
    """
    if scenario["hw_method"] == "percentile":
        q = int(scenario["hw_percentile"])

        if USE_ERA5_CLIMATOLOGY:
            era5_tmax_u = load_era5_tmax_series(usaf_u, wban_u, "urban")
            if era5_tmax_u is None or len(era5_tmax_u) == 0:
                return None, None, None, {
                    "fail_step": "ERA5_missing",
                    "missing_data": "ERA5 Tmax CSV 缺失或数据量不足"
                }

            hw_thr_series_raw = apply_doy_thr_to_index(
                u_tmax_all.index,
                compute_hw_doy_thr_era5(
                    era5_tmax_u,
                    q=q,
            ...
            valid_count_series = pd.Series(len(era5_tmax_u), index=u_tmax_all.index)
            ...
            hw_thr_series, valid_count_series = compute_hw_threshold_from_tmax(
                u_tmax_all,
                q=q,
                window_half_width=HW_WINDOW_HALF
            )
            hw_thr_series_raw = hw_thr_series.copy()
            hw_thr_series_corrected = hw_thr_series.copy()
            qm_diagnostics = {}
            era5_isd_bias_annual = np.nan
            bias_correction_method = "ISD_shortterm_threshold_no_ERA5"

        hw_thr_mean = (
            float(hw_thr_series.mean())
            if pd.Series(hw_thr_series).notna().any() else np.nan
        )
        if pd.isna(hw_thr_mean):
            return None, None, None, {
                "fail_step": "compute_hw_threshold_from_tmax",
                "missing_data": "城市Tmax数据过少，无法计算热浪阈值"
            }

        hw_mask_all = detect_heatwave(u_tmax_all, hw_thr_series)

        raw_reindexed = pd.Series(hw_thr_series_raw).reindex(u_tmax_all.index)
        corr_reindexed = pd.Series(hw_thr_series_corrected).reindex(u_tmax_all.index)

        threshold_info = {
            "hw_threshold_mean": hw_thr_mean,
            "hw_threshold_raw_mean": float(raw_reindexed.mean()) if raw_reindexed.notna().any() else np.nan,
            "hw_threshold_corrected_mean": float(corr_reindexed.mean()) if corr_reindexed.notna().any() else np.nan,
            "hw_threshold_qm_mean": float(corr_reindexed.mean()) if USE_QM_BIAS_CORRECTION and corr_reindexed.notna().any() else np.nan,
            ...
            "exceed_days_raw": int(
                (u_tmax_all > raw_reindexed).fillna(False).sum()
            ),
            "exceed_days_corrected": int(
                (u_tmax_all > corr_reindexed).fillna(False).sum()
            ),
            "exceed_days_qm": int(
                (u_tmax_all > corr_reindexed).fillna(False).sum()
            ) if USE_QM_BIAS_CORRECTION else np.nan,
        }
        return hw_mask_all, hw_thr_series, threshold_info, None

    # Absolute threshold scenarios.
    if scenario["abs_mode"] == "fixed":
        abs_thr = float(scenario["abs_threshold"])
    else:
        abs_thr = float(get_absolute_hw_threshold(continent))

    hw_mask_all = detect_heatwave_absolute(u_tmax_all, abs_thr)
    ...
    threshold_info = {
        ...
        "bias_correction_method": "not_applicable_absolute_threshold",
        "era5_isd_bias_annual": np.nan,
        ...
        "exceed_days_corrected": int((u_tmax_all > abs_thr).sum()),
        "exceed_days_qm": np.nan,
    }
    threshold_series = pd.Series(abs_thr, index=u_tmax_all.index)
    return hw_mask_all, threshold_series, threshold_info, None
    ...
    for period_name, (sub_u, sub_r) in periods.items():
        ...
        rec = {
            ...
            **_scenario_year_fields(scenario),
            ...
            "group": group,
            # UHI/UCI group definition is unchanged:
            # mean urban-minus-rural daily Tmax over common valid dates.
            "uhi_definition": "mean_urban_minus_rural_daily_Tmax",
            "uhi_classification_metric": "delta_tmax_ann",
            "continent": continent,
            "kg_code": kg_code,
...
def process_single_pair_scenario(args):
    ...
    hw_dates_warm = hw_dates_all & warm_common
    nhw_dates_warm = nhw_dates_all & warm_common
